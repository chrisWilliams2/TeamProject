...
public class editAnimalVet{
	...
	public static JTextField textBreed;
	...
	private void initialize() {
		...
		frmEditAnimal.setTitle("Edit Animal Account");
		...
		textType.setBounds(159, 170, 122, 33);
		frmEditAnimal.getContentPane().add(textType);
		
		textBreed = new JTextField();
		textBreed.setBackground(UIManager.getColor("Button.darkShadow"));
		textBreed.setEditable(false);
		textBreed.setColumns(10);
		textBreed.setBounds(159, 215, 122, 33);
		frmEditAnimal.getContentPane().add(textBreed);
		
		textGender = new JTextField();
		textGender.setEditable(false);
		textGender.setColumns(10);
		textGender.setBackground((Color) null);
		textGender.setBounds(159, 260, 122, 33);
		frmEditAnimal.getContentPane().add(textGender);
		...
		btnConfirm.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(action ==0)
				try{
					String query = ("Update animal set name= '"+textAnimalName.getText()+"' ,Type= '"+textType.getText()+"' , Breed= '"+textBreed.getText()+"',Gender='"+textGender.getText()+"' where aniID= '"+textFieldanimalID.getText()+"'" );
					PreparedStatement ps = (PreparedStatement) con.prepareStatement(query);
					ps.execute();
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				try{
					...
					String query = "select * from dbuseraccount.animal where aniID= '"+employeeid+"' ";
					PreparedStatement ps = (PreparedStatement) con.prepareStatement(query);
					ResultSet rs = ps.executeQuery();
					
					while (rs.next()){
						textFieldanimalID.setText(rs.getString("aniID"));
						textAnimalName.setText(rs.getString("name"));
						textType.setText(rs.getString("Type"));
						textBreed.setText(rs.getString("Breed"));
